...
Production-ready Helm chart for Kubernetes DaemonSets with modern best practices, security hardening, and comprehensive health checks.

**DaemonSets** ensure that a copy of a pod runs on all (or selected) nodes in your cluster. Perfect for cluster-wide services like:
- Log collectors (Fluentd, Filebeat)
- Monitoring agents (Prometheus Node Exporter, Datadog Agent)
- Storage daemons (Ceph, GlusterFS)
- Network proxies (Istio, Linkerd)
...
- **Node Targeting**: NodeSelector, affinity, and tolerations for precise pod placement
- **Network Security**: NetworkPolicy templates for ingress/egress control
- **Rolling Updates**: Configurable update strategy with maxUnavailable control
...
```bash
helm install my-daemon ./daemonset
```
