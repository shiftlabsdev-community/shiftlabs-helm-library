...
metadata:
  ...
  annotations:
    {{- if $isHAProxy }}
    {{- /* ============ HAProxy Annotations ============ */ -}}
    {{- if and .Values.ingress.ssl .Values.ingress.ssl.redirect }}
    haproxy.org/ssl-redirect: "true"
    {{- end }}
    {{- if or (and .Values.ingress.securityHeaders .Values.ingress.securityHeaders.enabled) (and .Values.ingress.cors .Values.ingress.cors.enabled) }}
    haproxy.org/response-set-header: |
      {{- if and .Values.ingress.securityHeaders .Values.ingress.securityHeaders.enabled }}
      ...
      {{- end }}
      {{- end }}
      {{- if and .Values.ingress.cors .Values.ingress.cors.enabled }}
      ...
      {{- end }}
    {{- end }}
    {{- if and .Values.ingress.proxy .Values.ingress.proxy.connectTimeout }}
    haproxy.org/timeout-connect: {{ printf "%ss" (toString .Values.ingress.proxy.connectTimeout) | quote }}
    {{- end }}
    {{- if and .Values.ingress.proxy (or .Values.ingress.proxy.readTimeout .Values.ingress.proxy.sendTimeout) }}
    ...
    {{- end }}
    {{- if and .Values.ingress.rateLimit .Values.ingress.rateLimit.enabled }}
    haproxy.org/rate-limit-requests: {{ .Values.ingress.rateLimit.rps | quote }}
    haproxy.org/rate-limit-period: "1s"
    {{- end }}
    {{- if and .Values.ingress.websocket .Values.ingress.websocket.enabled }}
    ...
    {{- end }}
    {{- if and .Values.ingress.ssl .Values.ingress.ssl.protocols }}
    nginx.ingress.kubernetes.io/ssl-protocols: {{ .Values.ingress.ssl.protocols | quote }}
    {{- end }}
    {{- if and .Values.ingress.ssl .Values.ingress.ssl.ciphers }}
    ...
    {{- end }}
    {{- end }}
    {{- if and .Values.ingress.proxy .Values.ingress.proxy.bodySize }}
